<?php 
/*
Plugin Name: SA-SoftScore - Live Score
...
class CPA_Theme_Options {
    ...
    add_options_page( 'Theme Options', 'Sa SoftScore', 'manage_options', __FILE__, array( $this, 'display_page' ) );
...
public function display_page() { 
    ...
    <div class="wrap">
        ...
        <h2>SASoft Livescore Theme Options</h2>
...
public function register_page_options() { 
    ...
    add_settings_section( 'cpa_section', 'SETTINGS', array( $this, 'display_section' ), __FILE__ ); // id, title, display cb, page
...
// Creating the widget 
class sascore_widget extends WP_Widget {
    ...
    __('SA Score Widget Widget', 'sascore_widget_domain'), 
      
    // Widget description
    array( 'description' => __( 'Sample widget based on WPBeginner Tutorial', 'sascore_widget_domain' ), ) 
    );
    }
    ...
    echo __( '', 'sascore_widget_domain' );
    $short_code= '[bzscore data-2="league" country-is="'.$country.'" league-is="'.$league.'"]';
    ...
    if ( isset( $instance[ 'title' ] ) ) {
        ...
        $title = __( 'New title', 'sascore_widget_domain' );
    }
    if ( isset( $instance[ 'country' ] ) ) {
        $country = $instance[ 'country' ];
        }
        else {
        $country = __( 'England', 'sascore_widget_domain' );
    }
    if ( isset( $instance[ 'league' ] ) ) {
        $league = $instance[ 'league' ];
        }
        else {
        $league = __( 'Premier League', 'sascore_widget_domain' );
    }
    // Widget admin form
    ?>
    <p>
    ...
    <p>
    <label for="<?php echo $this->get_field_id( 'country' ); ?>"><?php _e( 'Country:' ); ?> 
        <select class="widefat" id="<?php echo $this->get_field_id( 'country' ); ?>" name="<?php echo $this->get_field_name( 'country' ); ?>" type="text">
            <option value="england"<?php echo ($country=='england')?'selected':''; ?>>
                England
            </option>
            <option value="UEFA"<?php echo ($country=='UEFA')?'selected':''; ?>>
                UEFA
            </option>
            <option value="CONMEBOL"<?php echo ($country=='CONMEBOL')?'selected':''; ?>>
                CONMEBOL
            </option>
    ...
    <p>
    <label for="<?php echo $this->get_field_id( 'league' ); ?>"><?php _e( 'League:' ); ?> 
        <select class="widefat" id="<?php echo $this->get_field_id( 'league' ); ?>" name="<?php echo $this->get_field_name( 'league' ); ?>" type="text">
        <?php
            $types = array ('Premier League', 'Euro', 'Copa America', 'La Liga', 'Cup');
            foreach ($types as $option) {
                echo '<option value="' . $option . '"'
             . selected( $league==$option) . '>'. $option . '</option>';
            }?>
        </select>
    </label>
    </p>
    ...
    // Class sascore_widget ends here
    } 
     
     
    // Register and load the widget
    function wpb_load_widget() {
        register_widget( 'sascore_widget' );
    }
